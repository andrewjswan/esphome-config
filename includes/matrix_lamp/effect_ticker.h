...
void effectsTick()
{
  switch (currentMode)
  {
    ...
    case EFF_LOTUS:               DYNAMIC_DELAY_TICK { effTimer = millis(); FlowerRuta();                      }  break;  // ( 94U) Квітка лотоса (Червона рута)
    ...
    case EFF_UKRAINE:             DYNAMIC_DELAY_TICK { effTimer = millis(); Ukraine();                         }  break;  // (117U) Україна
  }
}
