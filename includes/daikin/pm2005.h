...
/*  
  	...
  	ESP_LOGD("custom", "P2 Frame length        : %02X", buf[2]);
  	ESP_LOGD("custom", "P3 Sensor situation    : %02X", buf[3]);
  	ESP_LOGD("custom", "P4 high byte PM1.0     : %02X", buf[4]);
  	ESP_LOGD("custom", "P5 low byte PM1.0      : %02X", buf[5]);
  	ESP_LOGD("custom", "P6 high byte PM2.5     : %02X", buf[6]);
  	ESP_LOGD("custom", "P7 low byte PM2.5      : %02X", buf[7]);
  	ESP_LOGD("custom", "P8 high byte PM10      : %02X", buf[8]);  	
  	ESP_LOGD("custom", "P9 low byte PM10       : %02X", buf[9]);
  	ESP_LOGD("custom", "P10 high byte M mode   : %02X", buf[10]);
  	ESP_LOGD("custom", "P11 low byte M mode    : %02X", buf[11]);
*/  
  	if(Sensor_Situation != buf[3])
  	{
	  	Sensor_Situation = buf[3];
  	...
  	pm1 = buf[4] * 0x100 + buf[5];
  	pm25 = buf[6] * 0x100 + buf[7];
